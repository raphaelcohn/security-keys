...
{
	local configuration_folder_name='security-keys'
	
	if [ -n "${XDG_CONFIG_HOME+set}" ]; then
		mkdir -m 0500 -p "$XDG_CONFIG_HOME"
		cd "$XDG_CONFIG_HOME" 1>/dev/null 2>/dev/null
			XDG_CONFIG_HOME="$(pwd)"
		cd - 1>/dev/null 2>/dev/null
	else if [ -n "${HOME+set}" ]; then
		mkdir -m 0500 -p "$HOME"
	...
	configuration_folder_path="$XDG_CONFIG_HOME"/"$configuration_folder_name"
